...
export default function DeliveryPage() {
  ...
  // Inicializar en 'available' solo si no hay pedido activo, sino en 'history'
  const [activeTab, setActiveTab] = useState<'history' | 'available' | 'transactions'>('available');
      ...
      if (savedSession) {
        try {
          ...
          if (sessionAge < maxAge && savedDriver && savedDriver.id) {
            // Verificar que el token esté guardado ANTES de restaurar la sesión
            const savedToken = localStorage.getItem('driverToken');
            if (!savedToken) {
              console.warn('⚠️ No hay token guardado, forzando re-login');
            ...
            setIsLoggedIn(true);
            // Verificar que el token sigue disponible después de establecer el estado
            setTimeout(() => {
              const tokenAfterState = localStorage.getItem('driverToken');
              if (!tokenAfterState) {
                console.error('❌ Token eliminado después de restaurar sesión!');
              } else {
                console.log('✅ Token sigue disponible después de restaurar sesión');
              }
            }, 100);
            ...
            try {
              ...
              // Si el error es de autenticación, forzar re-login
              if (err.message?.includes('Token') || err.message?.includes('401') || err.message?.includes('Unauthorized')) {
                console.warn('⚠️ Error de autenticación, forzando re-login');
                localStorage.removeItem('delivery_driver_session');
  ...
  useEffect(() => {
    if (driver && isLoggedIn) {
      ...
      // Asegurar que el token esté guardado (no eliminarlo si ya existe)
      const existingToken = localStorage.getItem('driverToken');
      if (!existingToken) {
        console.warn('⚠️ Token no encontrado al guardar sesión, pero driver está logueado');
      }
    } else if (!isLoggedIn && !driver) {
      // Solo eliminar token y sesión si explícitamente se hace logout
      // No eliminar si solo se está inicializando
      const savedSession = localStorage.getItem('delivery_driver_session');
      if (!savedSession) {
        // Solo eliminar si no hay sesión guardada (logout explícito)
        localStorage.removeItem('driverToken');
      }
    }
  }, [driver, isLoggedIn]);

  // Cambiar automáticamente a la pestaña de pedidos cuando hay un pedido activo
  useEffect(() => {
    if (currentOrder) {
      // Si hay un pedido activo y están en 'available', cambiar a 'history'
      if (activeTab === 'available') {
        setActiveTab('history');
      }
    } else {
      // Si no hay pedido activo y están en 'history' sin pedidos, cambiar a 'available'
      if (activeTab === 'history' && orderHistory.length === 0) {
        setActiveTab('available');
      }
    }
  }, [currentOrder, activeTab, orderHistory.length]);
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Guardar token en localStorage (el backend devuelve accessToken o access_token después de objectToSnakeCase)
      ...
      if (token) {
        ...
        console.error('❌ No se recibió token en la respuesta de login:', loginResponse);
  ...
  const startOrdersPolling = useCallback((driverId: string) => {
    ...
    // Polling optimizado: balance solo cada 60 segundos, pedidos cada 30 segundos
    ...
    ordersIntervalRef.current = setInterval(async () => {
      try {
        ...
        balanceCounter++;
        // Cargar balance solo cada 2 ciclos (cada 60 segundos)
        const shouldLoadBalance = balanceCounter % 2 === 0;
        
        await Promise.all([
          loadOrders(driverId, true), // silent update - cada 30 segundos
          loadOrderHistory(driverId, true), // cada 30 segundos
          shouldLoadBalance ? loadTransactions(driverId, true) : Promise.resolve() // cada 60 segundos
        ]);
        setLastUpdateTime(new Date());
      } catch (err) {
        console.error('Error en polling:', err);
      }
    }, 30000); // Cada 30 segundos (optimizado para 24/7)
  }, []);
  ...
  const loadOrders = useCallback(async (driverId: string, silent: boolean = false) => {
    try {
      ...
      if (!token) {
        console.warn('⚠️ loadOrders: No hay token disponible');
        if (!silent) {
          showToast('Sesión expirada. Por favor, inicia sesión nuevamente', 'error');
          setIsLoggedIn(false);
          setDriver(null);
  ...
  const handleAcceptOrder = useCallback(async (orderId: string) => {
    ...
    try {
      ...
      // Cambiar automáticamente a la pestaña de pedidos al aceptar
      setActiveTab('history');
  ...
  const handleCancelDelivery = useCallback(async () => {
    if (!driver || !currentOrder || !cancelReason) return;
    
    setLoading(true);
    setError('');
    try {
      await deliveryApi.cancelDelivery(driver.id, currentOrder.id, cancelReason, cancelNotes);
      
      showToast('Entrega cancelada. El repartidor fue liberado.', 'success');
      ...
      // Cambiar a pestaña de disponibles
      setActiveTab('available');
    } catch (err: any) {
      const errorMsg = err.message || 'Error al cancelar entrega';
      setError(errorMsg);
      showToast(errorMsg, 'error');
    } finally {
      setLoading(false);
    }
  }, [driver, currentOrder, cancelReason, cancelNotes, loadOrders, loadOrderHistory, loadTransactions, showToast]);
  ...
  const handleDeliverOrder = useCallback(async () => {
    ...
    try {
      ...
      // Cambiar a la pestaña según el resultado
      if (result && result.cash_collected && result.cash_collected > 0) {
        // Si hubo cobro en efectivo, mostrar movimientos
        setActiveTab('transactions');
      } else {
        // Si no hubo cobro, volver a disponibles (repartidor liberado)
        setActiveTab('available');
      }
    } catch (err: any) {
  ...
  const handleLogout = useCallback(() => {
    ...
    localStorage.removeItem('delivery_driver_session');
    localStorage.removeItem('driverToken');
    // Cambiar a la pestaña de disponibles al hacer logout
    setActiveTab('available');
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-lg border border-[#C7C7C7] p-1">
          <div className="flex space-x-1">
            ...
            <button
              onClick={() => {
                // Bloquear cambio a 'available' si hay un pedido activo
                if (currentOrder) {
                  showToast('⚠️ Completá o cancelá la entrega actual antes de ver pedidos disponibles', 'error');
                  return;
                }
                setActiveTab('available');
              }}
              disabled={!!currentOrder}
              className={`flex-1 px-4 py-3 rounded-lg font-medium text-sm transition-all duration-200 relative ${
                activeTab === 'available'
                  ? 'bg-[#111111] text-white border border-[#FFC300]'
                  : 'text-[#111111] hover:bg-gray-50 border border-transparent'
              } ${currentOrder ? 'opacity-50 cursor-not-allowed' : ''}`}
              title={currentOrder ? 'Completá la entrega actual para ver pedidos disponibles' : 'Ver pedidos disponibles'}
            >
              <div className="flex items-center justify-center space-x-2">
                <span className="text-base">📋</span>
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                  ...
                  {currentOrder ? '🔒' : availableOrders.length}
                </span>
              </div>
              {currentOrder && (
                          ...
                          {order.status === 'in_transit' && (
                            <>
                              <button
                                ...
                                className="w-full bg-[#111111] hover:bg-[#000000] text-white font-semibold py-4 rounded-lg transition-all duration-200 border-2 border-[#FFC300] hover:border-[#FFC300] flex items-center justify-center space-x-3 mb-2"
                              >
                                <span className="text-xl">✅</span>
                              ...
                              <button
                                ...
                                className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-4 rounded-lg transition-all duration-200 border-2 border-red-500 hover:border-red-600 flex items-center justify-center space-x-3"
                              >
                                <span className="text-xl">❌</span>
        ...
        {/* Tab: Pool Global de Tareas */}
        {activeTab === 'available' && (
          <div>
            {!isLoggedIn || !driver ? (
              <div className="bg-white rounded-lg border border-[#C7C7C7] p-12 text-center">
                <div className="text-5xl mb-4">🔐</div>
                <p className="text-base text-[#111111] font-medium mb-2">Debes iniciar sesión para ver tareas</p>
                <p className="text-sm text-[#C7C7C7]">Inicia sesión con tus credenciales de repartidor</p>
              </div>
            ) : (
              <TasksPanel 
                driverId={driver.id}
      ...
      {showCancelModal && currentOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg border border-[#C7C7C7] shadow-lg p-6 w-full max-w-md">
            <div className="text-center mb-5">
              <div className="rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-3 border-2 border-red-500 bg-white">
                <span className="text-3xl">❌</span>
              </div>
              <h3 className="text-xl font-bold text-[#111111] mb-2">
                Cancelar Entrega
            ...
            <div className="space-y-3 mb-5">
              {[
                { value: 'NO_ENTREGO_EL_CODIGO', label: 'No entregó el código' },
                { value: 'NO_ESTABA', label: 'No estaba' },
                { value: 'DIRECCION_INCORRECTA', label: 'Dirección incorrecta' },
                { value: 'CLIENTE_RECHAZO', label: 'Cliente rechazó el pedido' },
                ...
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg border-2 transition-all ${
                    cancelReason === option.value
                      ...
                      : 'border-[#C7C7C7] hover:border-red-300'
                  }`}
                >
                  {option.label}
                </button>
            ...
            {cancelReason === 'OTRO' && (
              <div className="mb-5">
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-[#C7C7C7] rounded-lg focus:outline-none focus:border-red-500"
                  rows={3}
                />
              </div>
            )}
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed border-2 border-red-500 hover:border-red-600"
              >
                {loading ? (
                  <span className="flex items-center justify-center space-x-2">
                    <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24">
