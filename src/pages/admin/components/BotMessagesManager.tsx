...
export default function BotMessagesManager() {
  ...
  const loadMessages = async () => {
    try {
      ...
      const data = await botMessagesApi.getAll();
      ...
      // Si hay error, intentar inicializar mensajes por defecto
      try {
        await initializeDefaultMessages();
      } catch (initError) {
  ...
  const initializeDefaultMessages = async () => {
    try {
      ...
      const messagesToInsert = defaultMessages.map(({ description, ...msg }) => ({
      ...
      const data = await botMessagesApi.createMany(messagesToInsert);
  ...
  const saveMessage = async (messageId: number | string) => {
    try {
      setSaving(true);

      await botMessagesApi.update(String(messageId), {
  ...
  const saveAllMessages = async () => {
    try {
      ...
      for (const msg of messages) {
        await botMessagesApi.update(String(msg.id), {
          message_text: msg.message_text
        });
      }
  ...
  const resetToDefaults = async () => {
    ...
    try {
      ...
      await botMessagesApi.deleteAll();
